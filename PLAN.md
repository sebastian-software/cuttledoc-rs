...
| Transcript-enhancement model/runtime bakeoff (#20) | In progress | The frozen Gemma challenge comparison early-rejects both safer contracts. A staged hosted preflight now freezes the same ten documents and 60 targets for GPT-5.6 Sol, Gemini 3.6 Flash, GPT-5.6 Terra, Kimi K3, and Claude Sonnet 5. All routes are provider-pinned ZDR with fallback disabled; repeat one has a `$17.49` deliberately overstated planning envelope, and only gate survivors receive repeat two. One `$0.008559` Gemini request identified and retired an undersized 768-token runner cap before quality execution; the corrected 2,048-token plan remains below the available credit even if every candidate reaches repeat two. |
