...
| Transcript-enhancement model/runtime bakeoff (#20) | In progress | The frozen Gemma challenge comparison early-rejects both safer contracts. A staged hosted preflight now freezes the same ten documents and 60 targets for GPT-5.6 Sol, Gemini 3.6 Flash, GPT-5.6 Terra, Kimi K3, and Claude Sonnet 5. All routes are provider-pinned ZDR with fallback disabled; repeat one has a `$11.19` deliberately overstated planning envelope, and only gate survivors receive repeat two. No hosted request is evidence until the preflight plan itself is committed. |
