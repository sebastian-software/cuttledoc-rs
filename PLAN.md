...
| Voxtral Realtime live streaming (#19) | In progress | The first repository-owned official-MLX boundary now validates the complete pinned model and proves hard queue capacity, 320 ms step budgets under 80 ms feeds, explicit backpressure, end-of-audio, and cancellation. Streaming frontend, causal encoder, Tekken/delay-conditioned decoder, exact transcript parity, then the official-MLX versus C/MPS comparison remain. |
