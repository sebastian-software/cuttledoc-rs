...
| Text generation | Official MLX through an owned model adapter | Preferred future embedded foundation; initial v3 backend deferred (ADR-0013) | The pinned 351 MB Qwen3 reference loaded in 428 ms, produced the first token in 197 ms, generated at 270 tokens/s, repeated deterministically, streamed token deltas, and stopped cooperatively. | The tiny model violated the surface-only lexical invariant and worsened the one development transcript from 2.56% to 5.13% WER. A safe multilingual model/prompt tuple and a repository-owned graph/KV-cache adapter are not yet proven. | Retain official MLX as the first embedded candidate; resume only with held-out per-language quality gates. Keep MLX-LM reference-only. |
...
| Text generation | Core ML stateful model | Deferred platform candidate | Current Core ML can represent stateful KV-cache-style models. | No selected converted multilingual instruction model has demonstrated a size, energy, compute, or deployment advantage over MLX; conversion becomes an additional maintained artifact boundary. | Reconsider only for a concrete model and measured deployment benefit. |
