...
| STT or text generation | Official MLX via owned C++ adapter | Advance as third first-class foundation; repository-owned boundary | Whisper Tiny proves CPU/Metal portability across two MLX releases; Qwen3-ASR proves a quantized 28-layer decoder, owned KV cache, multilingual audiobook quality, repeated Rust lifecycle, and cooperative cancellation; Voxtral proves a separately implemented causal frontend/encoder, delay-conditioned 26-layer GQA decoder, Tekken decoding, and exact Rust batch-transcript parity. | Held-out podcasts, long audio, incremental Voxtral input, aligned timestamps, clean cold start, energy, and pruned release packaging remain unproven. | Move the proven Voxtral graph behind its bounded task ABI, then continue the target-domain benchmark. |
