...
| STT or text generation | Official MLX via owned C++ adapter | Advance as third first-class foundation; repository-owned boundary | Whisper Tiny proves CPU/Metal portability across two MLX releases; Qwen3-ASR proves a quantized 28-layer decoder, owned KV cache, multilingual audiobook quality, repeated Rust lifecycle, and cooperative cancellation; Voxtral proves a separately implemented persistent causal frontend/encoder/decoder session, bounded live input, append-only updates, and exact streaming-oracle text at 480/2,400 ms. | Held-out podcasts, long audio, aligned timestamps, clean cold start, energy, 80 ms stall reduction, and pruned release packaging remain unproven. | Carry the stable 320 ms Voxtral path into held-out/common-engine gates and compare lifecycle and packaging with pure C/MPS. |
