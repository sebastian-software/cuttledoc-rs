...
| STT or text generation | Official MLX via owned C++ adapter | Accepted as the third first-class foundation; repository-owned model boundary (ADR-0012) | Whisper Tiny proves CPU/Metal portability across two MLX releases; Qwen3-ASR proves a quantized 28-layer decoder, owned KV cache, multilingual audiobook quality, repeated Rust lifecycle, and cooperative cancellation; Voxtral proves a separately implemented persistent causal frontend/encoder/decoder session, bounded live input, append-only updates, and exact streaming-oracle text at 480/2,400 ms. | Held-out podcasts, long audio, aligned timestamps, clean cold start, energy, 80 ms stall reduction, and pruned release packaging remain unproven. | Carry the stable 320 ms Voxtral path into held-out/common-engine gates. |
| STT | `antirez/voxtral.c` with Metal/MPS | Reference-only comparison control | A 246 KB executable links only Apple frameworks; the BF16 model produces a near-matching German smoke transcript and demonstrates a readable dependency-free inference graph. | The 8.87 GB model and 8.43 GB Metal cache erase much of the delivery advantage. Synchronous `feed()` has no cancellation, queue bound, backpressure, busy status, or synchronization. The eleven-day, no-release-tag project fails ADR-0005. | Retain as implementation/packaging evidence; do not depend on or fork it. |
...
| Text generation | Official MLX through an owned model adapter | Serious embedded finalist after model-quality selection (ADR-0013) | The pinned 351 MB Qwen3 reference loaded in 428 ms, produced the first token in 197 ms, generated at 270 tokens/s, repeated deterministically, streamed token deltas, and stopped cooperatively. | The tiny model violated the surface-only lexical invariant and worsened the one development transcript from 2.56% to 5.13% WER. A safe multilingual model/prompt tuple and a repository-owned graph/cache adapter are not yet proven. | Use MLX-LM only as the common pinned reference for #20; port the quality survivor through a narrow owned adapter and compare it with Core ML. |
| Text generation | llama.cpp / GGUF | Mature embedded fallback; not a dependency | Mature C API, Apple Metal path, broad model availability, local server, and cross-platform delivery. | Adds a second native runtime and normally duplicates MLX safetensors as GGUF; no measured product advantage currently pays for that packaging/cache matrix. | Promote only if cross-platform embedded inference or GGUF availability becomes a concrete requirement. |
| Text generation | OpenAI remote HTTP provider | Independently implementable repository-owned adapter | No embedded runtime or model distribution; maps to the same ordered generation stream. | Credentials, privacy, cost, rate limits, cancellation, network errors, and opaque model revisions remain. | May proceed independently from local model selection. |
| Text generation | Ollama local HTTP provider | Independently implementable repository-owned adapter | Preserves the Cuttledoc 2 local-provider route without making Ollama the embedded runtime or cache owner. | Process discovery, provider-managed models/cache, request cancellation, capabilities, and replacement need explicit mapping. | May proceed independently from OpenAI and embedded work. |
| Text generation | mistral.rs or Candle / Metal-native candidate | Reference only after #7 review | Active implementations and useful Rust/Metal/model-format prior art. | Broad runtime, model, build, and transitive ownership remains disproportionate to this narrow optional task. | Reconsider only with a measured advantage over the official-MLX lead. |
| Text generation | Core ML stateful model | Serious embedded finalist after model-quality selection (ADR-0013) | Current Core ML can represent stateful KV-cache-style models and compressed weights; it is not limited to tiny models. | Conversion becomes an additional maintained artifact boundary, and Core ML does not guarantee Neural Engine placement for an arbitrary graph. | Convert the same quality survivor as the MLX path; measure parity, actual compute placement, size, energy procedure, delivery, and lifecycle. |
...
| TTS | Qwen3-TTS VoiceDesign, Voxtral TTS BF16, and bounded KugelAudio through pinned MLX-Audio | Serious July 2026 calibration candidates; reference runner first | Pinned Apple Silicon implementations can establish per-language model quality, voice variance, audio, timing, and delivery evidence without predetermining the Rust API. Historical Qwen3-TTS 0.6B and Voxtral 4-bit runs already prove the reference workflow. | New converted model/runtime revisions and digests, listening evidence, Python ownership, dependency breadth, packaging, cancellation, and direct-official-MLX cost remain unmeasured. Voxtral weights are reference-only under their current license. | Pin the current Qwen VoiceDesign and Voxtral BF16 paths, run the bounded German/English calibration, qualify KugelAudio only on one German passage, then compare retaining the selected MLX-Audio subset with a narrow owned adapter over official MLX. |
