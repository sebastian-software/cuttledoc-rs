...
| [mlx-audio](https://github.com/Blaizzy/mlx-audio) | Phase 5 TTS candidate; pinned reference runner first | The active MIT project supplies practical Apple Silicon implementations for Qwen3-TTS and Chatterbox in addition to its broad audio surface. Issue #13 starts with audited commit `64e8416c303fb3b3463dab8eb4ebd78c55a87c1a` as a disposable reference runner, but does not reject the library as a future product dependency. Measurements must compare retaining the selected MLX-Audio subset with owning a narrow model adapter directly over official MLX. Production adoption still requires ADR-0005 evidence for exact packages, Python/Swift/runtime ownership, model delivery, updates, binary/transitive cost, and replacement. Until that review passes, it remains absent from product manifests and release artifacts. |
