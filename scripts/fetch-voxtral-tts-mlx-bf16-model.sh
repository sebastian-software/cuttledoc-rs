...
manifest="$repo_root/spikes/tts-calibration/voxtral-tts-4b-mlx-bf16.json"
model_dir=${1:-/tmp/cuttledoc-voxtral-tts-4b-mlx-bf16}
repository=$(
  node -e 'console.log(require(process.argv[1]).artifact.repository)' "$manifest"
)
revision=$(
  node -e 'console.log(require(process.argv[1]).artifact.revision)' "$manifest"
)
