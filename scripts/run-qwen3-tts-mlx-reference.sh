...
model_dir=${CUTTLEDOC_QWEN3_TTS_MODEL_DIR:-/tmp/cuttledoc-qwen3-tts-0.6b-customvoice-mlx-bf16}
text_dir=${CUTTLEDOC_TTS_TEXT_DIR:-/tmp/cuttledoc-synthetic-roundtrip-pilot-1-verified}
output_dir=${CUTTLEDOC_QWEN3_TTS_OUTPUT_DIR:-/tmp/cuttledoc-qwen3-tts-reference-run}
source_revision=$(git -C "$repo_root" rev-parse HEAD)

if [[ ! -d "$model_dir" ]]; then
  echo "Model directory not found: $model_dir" >&2
  echo "Run scripts/fetch-qwen3-tts-mlx-model.sh first." >&2
  exit 1
fi
