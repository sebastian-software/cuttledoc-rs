...
if (actual.cache_offset_after_prefill !== oracle.prompt_length) {
  errors.push(
...
for (const field of ['first_top_tokens', 'second_top_tokens']) {
  ...
  const overlap = observed.filter(({ token }) => expectedSet.has(token)).length;
...
if (actual.generated_tokens !== undefined) {
  ...
  JSON.stringify(
    {
      ...
      exact: [
        ...
        ...(actual.generated_tokens === undefined
          ? []
